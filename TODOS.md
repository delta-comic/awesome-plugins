...
- [ ] 彻底重构当前的控制系统
  - [ ] 重构issue控制模版，使得用户可以通过markdown控制包注册行为
  - [ ] 重构回答体系，使其更加明显(使用emoji)且附加日志(默认折叠)
  - [ ] 优化鉴权逻辑，使得不要让任何人都可以随意修改注册
- [ ] 对于同在github的插件
  - [ ] 需要通过定时任务扫描发行版的最新版本，给出最新正式发行版文件中`manifest.json`文件的链接(如果存在)
  - [ ] 需要通过定时任务扫描，选出最近提交的5个，在本仓库README展示，形如
    ```md
    ### ${仓库名称}
    ...
    **下载:**
    ...
    ```
- [ ] 规范化数据结构，编写json schema，使用类型检测库检查写入文件类型是否正确
- [ ] 测试使用`bun test`
